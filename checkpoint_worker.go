...
// LoadLastLSN retrieves the last LSN for a given table from the cdc_offsets table.
func (cw *CheckpointWorker) LoadLastLSN(req LoadLastLSNRequest) LoadLastLSNResponse {
...
// SaveLastLSN updates the last LSN and timestamp for a given table in the cdc_offsets table.
func (cw *CheckpointWorker) SaveLastLSN(req SaveLastLSNRequest) SaveLastLSNResponse {
	query := `
...
func (cw *CheckpointWorker) Start() {
	go func() {
		...
		_, err := cw.nc.Subscribe("checkpoint.load", cw.handleLoadLastLSN)
		if err != nil {
